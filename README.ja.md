...
> UI は日本語と英語に対応していて、デフォルトは日本語です。SNS でシェアされた時に言語の一貫性を保つため、`Accept-Language` は意図的に無視しています。`?lang=en` または右上の言語切替リンクで英語に切り替えられます。
...
- **間違いへの対処**: 原因が悪意でも単純なミスでも、結果を変質させるという意味では同じなので、両方に対処します:
  ...
  - ID は紛らわしい字 (`0/o`, `1/i/l` など) を避け、同じ文字位置で QWERTY キーボード上の隣接キーが使われないように作られています。**入力ミスはフォームの正規表現バリデーションで弾かれ**、回答者に再入力を促せます。
...
これに対応する正規表現 (= ID 認識パターン) をフォームのバリデーションに設定すれば、入力ミスを **フォーム側で誤りとして検出** し、回答者に再入力を促せます。
